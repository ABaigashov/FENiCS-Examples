...
# Можно взять любую монотннную функцию, которая в нуле принимает значение g0,
# а при x=R0 равна нулю
g=g0*(1-x*x/(R0*R0))

# создание файла H.txt, в которой записываются построчно x и y-коорнннаты узлов
# и значение энтальпии в этих узлах.
...
for k in range(8):
    ...
    fig=plt.figure(figsize=(8,6), facecolor='pink', frameon=True)
    plt.plot(data_x,data_y1)
    plt.savefig('results/Profile1-2.png')
    plt.show()

    fig=plt.figure(figsize=(8,6), facecolor='pink', frameon=True)
    plt.plot(data_x,data_y2)
    plt.savefig('results/Profile2-2.png')
    plt.show()
